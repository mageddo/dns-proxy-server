...
public interface Winsvc {
  ...
  interface Handler extends StdCallLibrary.StdCallCallback {
     ...
     *                   <tr><td>SERVICE_CONTROL_CONTINUE<br>0x00000003</td><td>Notifies a paused
     *                   service that it should resume.</td></tr>
     *                   <tr><td>SERVICE_CONTROL_INTERROGATE<br>0x00000004</td><td>Notifies a
     *                   service that it should report its current status information to the
     *                   service control manager.<br>The handler should simply return NO_ERROR;
     *                   the SCM is aware of the current state of the service.</td></tr>
     *                   <tr><td>SERVICE_CONTROL_NETBINDADD<br>0x00000007</td><td>Notifies a
     *                   network service that there is a new component for binding. The service
     *                   should bind to the new component.<br>Applications should use Plug and
     *                   Play functionality instead.</td></tr>
     *                   <tr><td>SERVICE_CONTROL_NETBINDDISABLE<br>0x0000000A</td><td>Notifies a
     *                   network service that one of its bindings has been disabled. The service
     *                   should reread its binding information and remove the binding
     *                   .<br>Applications should use Plug and Play functionality instead.</td></tr>
     *                   <tr><td>SERVICE_CONTROL_NETBINDENABLE<br>0x00000009</td><td>Notifies a
     *                   network service that a disabled binding has been enabled. The service
     *                   should reread its binding information and add the new binding
     *                   .<br>Applications should use Plug and Play functionality instead.</td></tr>
     *                   <tr><td>SERVICE_CONTROL_NETBINDREMOVE<br>0x00000008</td><td>Notifies a
     *                   network service that a component for binding has been removed. The
     *                   service should reread its binding information and unbind from the
     *                   removed component.<br>Applications should use Plug and Play
     *                   functionality instead.</td></tr>
     *                   <tr><td>SERVICE_CONTROL_PARAMCHANGE<br>0x00000006</td><td>Notifies a
     *                   service that its startup parameters have changed. The service should
     *                   reread its startup parameters.</td></tr>
     *                   <tr><td>SERVICE_CONTROL_PAUSE<br>0x00000002</td><td>Notifies a service
     *                   that it should pause.</td></tr>
     *                   <tr><td>SERVICE_CONTROL_SHUTDOWN<br>0x00000005</td><td>Notifies a
     *                   service that the system is shutting down so the service can perform
     *                   cleanup tasks.<br>If a service accepts this control code, it must stop
     *                   after it performs its cleanup tasks and return NO_ERROR. After the SCM
     *                   sends this control code, it will not send other control codes to the
     *                   service.</td></tr>
     *                   <tr><td>SERVICE_CONTROL_STOP<br>0x00000001</td><td>Notifies a service
     *                   that it should stop.<br>If a service accepts this control code, it must
     *                   stop upon receipt and return NO_ERROR. After the SCM sends this control
     *                   code, it does not send other control codes.<br>Windows XP:  If the
     *                   service returns NO_ERROR and continues to run, it continues to receive
     *                   control codes. This behavior changed starting with Windows Server 2003
     *                   and Windows XP with SP2.</td></tr>
     *                   </table>
     *
     *                   <p>This parameter can also be a user-defined control code, as described
     *                   in the following table.</p>
     *
     *                   <table>
     *                   <tr><th>Control code</th><th>Meaning</th></tr>
     *                   <tr><td>Range 128 to 255.</td><td>The service defines the action
     *                   associated with the control code.</td></tr>
     *                   </table>
     */
  ...
  interface HandlerEx extends StdCallLibrary.StdCallCallback {
     ...
     *                    <table>
     *                    <tr><th>Control code</th><th>Meaning</th></tr>
     *                    <tr><td>SERVICE_CONTROL_CONTINUE<br>0x00000003</td><td>Notifies a
     *                    paused service that it should resume.</td></tr>
     *                    <tr><td>SERVICE_CONTROL_INTERROGATE<br>0x00000004</td><td>Notifies a
     *                    service that it should report its current status information to the
     *                    service control manager.<br>The handler should simply return NO_ERROR;
     *                    the SCM is aware of the current state of the service.</td></tr>
     ...
     *                    network service that one of its bindings has been disabled. The service
     *                    should reread its binding information and remove the binding
     *                    .<br>Applications should use Plug and Play functionality instead
     *                    .</td></tr>
     *                    <tr><td>SERVICE_CONTROL_NETBINDENABLE<br>0x00000009</td><td>Notifies a
     *                    network service that a disabled binding has been enabled. The service
     *                    should reread its binding information and add the new binding
     ...
     *                    <tr><td>SERVICE_CONTROL_PAUSE<br>0x00000002</td><td>Notifies a service
     *                    that it should pause.</td></tr>
     *                    <tr><td>SERVICE_CONTROL_PRESHUTDOWN<br>0x0000000F</td><td>Notifies a
     *                    service that the system will be shutting down. Services that need
     *                    additional time to perform cleanup tasks beyond the tight time
     *                    restriction at system shutdown can use this notification. The service
     *                    control manager sends this notification to applications that have
     *                    registered for it before sending a SERVICE_CONTROL_SHUTDOWN
     *                    notification to applications that have registered for that notification
     *                    .<br>A service that handles this notification blocks system shutdown
     *                    until the service stops or the preshutdown time-out interval specified
     *                    through SERVICE_PRESHUTDOWN_INFO expires. Because this affects the user
     *                    experience, services should use this feature only if it is absolutely
     *                    necessary to avoid data loss or significant recovery time at the next
     *                    system start.<br>Windows Server 2003 and Windows XP:  This value is not
     *                    supported.</td></tr>
     *                    <tr><td>SERVICE_CONTROL_SHUTDOWN<br>0x00000005</td><td>Notifies a
     *                    service that the system is shutting down so the service can perform
     *                    cleanup tasks.<br>If a service accepts this control code, it must stop
     ...
     *                    </table>
     *
     *                    <p>This parameter can also be one of the following extended control
     *                    codes. Note that these control codes are not supported by the
     *                    {@link Handler} function.</p>
     *
     *                    <table>
     *                    <tr><th>Control code</th><th>Meaning</th></tr>
     *                    <tr><td>SERVICE_CONTROL_DEVICEEVENT<br>0x0000000B</td><td>Notifies a
     *                    service of device events. (The service must have registered to receive
     *                    these notifications using the RegisterDeviceNotification function.) The
     *                    dwEventType and lpEventData parameters contain additional information
     *                    .</td></tr>
     *                    <tr><td>SERVICE_CONTROL_HARDWAREPROFILECHANGE<br>0x0000000C</td><td
     *                    >Notifies a service that the computer's hardware profile has changed.
     *                    The dwEventType parameter contains additional information.</td></tr>
     *                    <tr><td>SERVICE_CONTROL_POWEREVENT<br>0x0000000D</td><td>Notifies a
     *                    service of system power events. The dwEventType parameter contains
     *                    additional information. If dwEventType is PBT_POWERSETTINGCHANGE, the
     *                    lpEventData parameter also contains additional information.</td></tr>
     *                    <tr><td>SERVICE_CONTROL_SESSIONCHANGE<br>0x0000000E</td><td>Notifies a
     *                    service of session change events. Note that a service will only be
     *                    notified of a user logon if it is fully loaded before the logon attempt
     *                    is made. The dwEventType and lpEventData parameters contain additional
     *                    information. </td></tr>
     *                    <tr><td>SERVICE_CONTROL_TIMECHANGE<br>0x00000010</td><td>Notifies a
     *                    service that the system time has changed. The lpEventData parameter
     *                    contains additional information. The dwEventType parameter is not used
     *                    .<br>Windows Server 2008, Windows Vista, Windows Server 2003 and
     *                    Windows XP:  This control code is not supported.</td></tr>
     *                    <tr><td>SERVICE_CONTROL_TRIGGEREVENT<br>0x00000020</td><td>Notifies a
     *                    service registered for a service trigger event that the event has
     *                    occurred.<br>Windows Server 2008, Windows Vista, Windows Server 2003
     *                    and Windows XP:  This control code is not supported.</td></tr>
     *                    <tr><td>SERVICE_CONTROL_USERMODEREBOOT<br>0x00000040</td><td>Notifies a
     *                    service that the user has initiated a reboot.<br>Windows Server 2008
     *                    R2, Windows 7, Windows Server 2008, Windows Vista, Windows Server 2003
     *                    and Windows XP:  This control code is not supported.</td></tr>
     *                    </table>
     *
     *                    <p>This parameter can also be a user-defined control code, as described
