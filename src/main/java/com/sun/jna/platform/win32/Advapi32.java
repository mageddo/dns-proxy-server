...
public interface Advapi32 extends StdCallLibrary {
   ...
   *
   * @param lpSystemName            Specifies the name of the system.
   * @param lpAccountName           Specifies the account name.
   * @param Sid                     Receives the SID structure that corresponds to the account
   *                                name pointed to by the lpAccountName parameter.
   * @param cbSid                   On input, this value specifies the size, in bytes, of the Sid
   *                                buffer. If the function fails because the buffer is too small
   *                                or if cbSid is zero, this variable receives the required
   *                                buffer size.
   * @param ReferencedDomainName    Receives the name of the domain where the account name is
   *                                found.
   * @param cchReferencedDomainName On input, this value specifies the size, in TCHARs, of the
   ...
   * @param peUse                   SID_NAME_USE enumerated type that indicates the type of the
   ...
   * @param lpName                  Buffer that receives a null-terminated string that contains
   *                                the account name that corresponds to the lpSid parameter.
   * @param cchName                 On input, specifies the size, in TCHARs, of the lpName buffer.
   ...
   * @param ReferencedDomainName    Pointer to a buffer that receives a null-terminated string
   *                                that contains the name of the domain where the account name
   *                                was found.
   * @param cchReferencedDomainName On input, specifies the size, in TCHARs, of the
   ...
   *                                including the terminating null character.
   * @param peUse                   Pointer to a variable that receives a SID_NAME_USE value that
   ...
   *
   * @param Sid       The SID structure to be converted.
   * @param StringSid Pointer to a variable that receives a pointer to a
   ...
   * @see <A HREF="http://msdn.microsoft.com/en-us/library/aa376399(VS.85)
   * .aspx">ConvertSidToStringSid</A>
   */
   ...
   * @param Sid       Receives a pointer to the converted SID. To free the returned buffer, call
   *                  the LocalFree function.
   * @return {@code true} if the function was successful - call {@code GetLastError()}
   * to check failure reason
   * @see <A HREF="http://msdn.microsoft.com/en-us/library/aa376402(VS.85)
   * .aspx">ConvertStringSidToSid</A>
   */
   ...
   *
   * @param pSid Pointer to the SID structure to validate. This parameter
   *             cannot be NULL.
   ...
   *
   * @param pSid1 A pointer to the first SID structure to compare. This structure is assumed to
   *              be valid.
   * @param pSid2 A pointer to the second SID structure to compare. This structure is assumed to
   *              be valid.
   ...
   *
   * @param pSid             SID to test.
   * @param wellKnownSidType Member of the WELL_KNOWN_SID_TYPE enumeration to compare with
   *                         the SID at pSid.
   ...
   *
   * @param wellKnownSidType Member of the WELL_KNOWN_SID_TYPE enumeration that specifies
   *                         what the SID will identify.
   * @param domainSid        Pointer to a SID that identifies the domain control to use
   *                         when creating the SID. Pass NULL to use the local computer.
   * @param pSid             Pointer to memory where CreateWellKnownSid will store the new
   *                         SID.
   * @param cbSid            Pointer to a DWORD that contains the number of bytes available
   ...
   *
   * @param pSecurityDescriptor A pointer to a SECURITY_DESCRIPTOR structure that the function
   *                            initializes.
   * @param dwRevision          The revision level to assign to the security descriptor. This
   *                            parameter
  ...
  /**
   * The GetSecurityDescriptorControl function retrieves a security descriptor control and
   * revision information.
   *
   * @param pSecurityDescriptor A pointer to a SECURITY_DESCRIPTOR structure whose control and
   *                            revision
   *                            information the function retrieves.
   * @param pControl            A pointer to a SECURITY_DESCRIPTOR_CONTROL structure that
   *                            receives the security descriptor's
   *                            control information.
   * @param lpdwRevision        A pointer to a variable that receives the security descriptor's
   *                            revision value.
   *                            This value is always set, even when GetSecurityDescriptorControl
   *                            returns an error.
   * @return If the function succeeds, the return value is nonzero. If the
   * function fails, the return value is zero. For extended error
   * information, call GetLastError.
   */
  boolean GetSecurityDescriptorControl(SECURITY_DESCRIPTOR pSecurityDescriptor,
   ...
   * The function can set only the control
   * bits that relate to automatic inheritance of ACEs. To set the other control bits of a
   * security descriptor, use the functions,
   * such as SetSecurityDescriptorDacl, for modifying the components of a security descriptor.
   *
   * @param pSecurityDescriptor   A pointer to a SECURITY_DESCRIPTOR structure whose control and
   *                              revision information are set.
   * @param ControlBitsOfInterest A SECURITY_DESCRIPTOR_CONTROL mask that indicates the control
   *                              bits to set.
   * @param ControlBitsToSet      SECURITY_DESCRIPTOR_CONTROL mask that indicates the new values
   *                              for the control bits specified by the ControlBitsOfInterest mask.
   * @return If the function succeeds, the return value is nonzero. If the
   * function fails, the return value is zero. For extended error
   * information, call GetLastError.
   */
  boolean SetSecurityDescriptorControl(SECURITY_DESCRIPTOR pSecurityDescriptor,
      short ControlBitsOfInterest, short ControlBitsToSet);

  /**
   * The GetSecurityDescriptorOwner function retrieves the owner information from a security
   * descriptor.
   *
   * @param pSecurityDescriptor A pointer to a SECURITY_DESCRIPTOR structure whose owner
   *                            information the function retrieves.
   * @param pOwner              A pointer to a pointer to a security identifier (SID) that
   *                            identifies the owner when the function returns.
   *                            If the security descriptor does not contain an owner, the
   *                            function sets the pointer pointed to by pOwner
   *                            to NULL and ignores the remaining output parameter,
   *                            lpbOwnerDefaulted. If the security descriptor contains an owner,
   *                            the function sets the pointer pointed to by pOwner to the address
   *                            of the security descriptor's owner SID
   *                            and provides a valid value for the variable pointed to by
   *                            lpbOwnerDefaulted.
   * @param lpbOwnerDefaulted   A pointer to a flag that is set to the value of the
   *                            SE_OWNER_DEFAULTED flag in the SECURITY_DESCRIPTOR_CONTROL
   *                            structure when the function returns. If the value stored in the
   *                            variable pointed to by the pOwner parameter is
   *                            NULL, no value is set.
   * @return If the function succeeds, the return value is nonzero. If the
   * function fails, the return value is zero. For extended error
   * information, call GetLastError.
   */
  boolean GetSecurityDescriptorOwner(SECURITY_DESCRIPTOR pSecurityDescriptor,
      PSIDByReference pOwner, BOOLByReference lpbOwnerDefaulted);

  /**
   * The SetSecurityDescriptorOwner function sets the owner information of an absolute-format
   * security descriptor. It replaces
   * any owner information already present in the security descriptor.
   *
   * @param pSecurityDescriptor A pointer to the SECURITY_DESCRIPTOR structure whose owner is set
   *                            by this function. The function replaces any existing
   *                            owner with the new owner.
   * @param pOwner              A pointer to a SID structure for the security descriptor's new
   *                            primary owner. The SID structure is referenced by, not
   *                            copied into, the security descriptor. If this parameter is NULL,
   *                            the function clears the security descriptor's owner
   *                            information. This marks the security descriptor as having no owner.
   * @param bOwnerDefaulted     Indicates whether the owner information is derived from a default
   *                            mechanism. If this value is TRUE, it is default information.
   *                            The function stores this value as the SE_OWNER_DEFAULTED flag in
   *                            the SECURITY_DESCRIPTOR_CONTROL structure. If this parameter
   *                            is zero, the SE_OWNER_DEFAULTED flag is cleared.
   * @return If the function succeeds, the return value is nonzero. If the
   * function fails, the return value is zero. For extended error
   * information, call GetLastError.
   */
  boolean SetSecurityDescriptorOwner(SECURITY_DESCRIPTOR pSecurityDescriptor, PSID pOwner,
      boolean bOwnerDefaulted);

  /**
   * The GetSecurityDescriptorGroup function retrieves the primary group information from a
   * security descriptor.
   *
   * @param pSecurityDescriptor A pointer to a SECURITY_DESCRIPTOR structure whose primary group
   *                            information the function retrieves.
   * @param pGroup              A pointer to a pointer to a security identifier (SID) that
   *                            identifies the primary group when the function
   *                            returns. If the security descriptor does not contain a primary
   *                            group, the function sets the pointer
   *                            pointed to by pGroup to NULL and ignores the remaining output
   *                            parameter, lpbGroupDefaulted. If the
   *                            security descriptor contains a primary group, the function sets
   *                            the pointer pointed to by pGroup to the
   *                            address of the security descriptor's group SID and provides a
   *                            valid value for the variable pointed to
   *                            by lpbGroupDefaulted.
   * @param lpbGroupDefaulted   A pointer to a flag that is set to the value of the
   *                            SE_GROUP_DEFAULTED flag in the
   *                            SECURITY_DESCRIPTOR_CONTROL structure when the function returns.
   ...
   * information, call GetLastError.
   */
  boolean GetSecurityDescriptorGroup(SECURITY_DESCRIPTOR pSecurityDescriptor,
      PSIDByReference pGroup, BOOLByReference lpbGroupDefaulted);

  /**
   * The SetSecurityDescriptorGroup function sets the primary group information of an
   * absolute-format security descriptor, replacing
   * any primary group information already present in the security descriptor.
   *
   * @param pSecurityDescriptor A pointer to the SECURITY_DESCRIPTOR structure whose primary
   *                            group is set by this function. The function replaces
   *                            any existing primary group with the new primary group.
   * @param pGroup              A pointer to a SID structure for the security descriptor's new
   *                            primary group. The SID structure is referenced by, not copied
   *                            into, the security descriptor. If this parameter is NULL, the
   *                            function clears the security descriptor's primary group
   *                            information. This marks the security descriptor as having no
   *                            primary group.
   * @param bGroupDefaulted     Indicates whether the primary group information was derived from
   *                            a default mechanism. If this value is TRUE, it is default
   *                            information, and the function stores this value as the
   *                            SE_GROUP_DEFAULTED flag in the SECURITY_DESCRIPTOR_CONTROL
   *                            structure.
   ...
   * information, call GetLastError.
   */
  boolean SetSecurityDescriptorGroup(SECURITY_DESCRIPTOR pSecurityDescriptor, PSID pGroup,
      boolean bGroupDefaulted);

  /**
   * The GetSecurityDescriptorDacl function retrieves a pointer to the discretionary access
   * control list (DACL) in
   * a specified security descriptor.
   *
   * @param pSecurityDescriptor A pointer to the SECURITY_DESCRIPTOR structure that contains the
   *                            DACL. The function retrieves a pointer to it.
   * @param bDaclPresent        A pointer to a value that indicates the presence of a DACL in the
   *                            specified security descriptor. If
   *                            lpbDaclPresent is TRUE, the security descriptor contains a DACL,
   *                            and the remaining output parameters in this
   *                            function receive valid values. If lpbDaclPresent is FALSE, the
   *                            security descriptor does not contain a DACL,
   *                            and the remaining output parameters do not receive valid values.
   *                            A value of TRUE for lpbDaclPresent does not
   *                            mean that pDacl is not NULL. That is, lpbDaclPresent can be TRUE
   *                            while pDacl is NULL, meaning that a NULL
   *                            DACL is in effect. A NULL DACL implicitly allows all access to an
   *                            object and is not the same as an empty DACL.
   *                            An empty DACL permits no access to an object. For information
   *                            about creating a proper DACL, see Creating a DACL.
   * @param pDacl               A pointer to a pointer to an access control list (ACL). If a DACL
   *                            exists, the function sets the pointer pointed
   *                            to by pDacl to the address of the security descriptor's DACL. If
   *                            a DACL does not exist, no value is stored.
   *                            If the function stores a NULL value in the pointer pointed to by
   *                            pDacl, the security descriptor has a NULL DACL.
   *                            A NULL DACL implicitly allows all access to an object.
   *                            If an application expects a non-NULL DACL but encounters a NULL
   *                            DACL, the application should fail securely and
   *                            not allow access.
   * @param bDaclDefaulted      A pointer to a flag set to the value of the SE_DACL_DEFAULTED
   *                            flag in the SECURITY_DESCRIPTOR_CONTROL structure
   *                            if a DACL exists for the security descriptor. If this flag is
   *                            TRUE, the DACL was retrieved by a default mechanism;
   *                            if FALSE, the DACL was explicitly specified by a user.
   * @return If the function succeeds, the return value is nonzero. If the
   * function fails, the return value is zero. For extended error
   * information, call GetLastError.
   */
  boolean GetSecurityDescriptorDacl(SECURITY_DESCRIPTOR pSecurityDescriptor,
      BOOLByReference bDaclPresent, PACLByReference pDacl, BOOLByReference bDaclDefaulted);

  /**
   * The SetSecurityDescriptorDacl function sets information in a discretionary access control
   * list (DACL).
   * If a DACL is already present in the security descriptor, the DACL is replaced.
   *
   * @param pSecurityDescriptor A pointer to the SECURITY_DESCRIPTOR structure to which the
   *                            function adds the DACL. This
   *                            security descriptor must be in absolute format, meaning that its
   *                            members must be pointers
   *                            to other structures, rather than offsets to contiguous data.
   * @param bDaclPresent        A flag that indicates the presence of a DACL in the security
   *                            descriptor. If this parameter
   *                            is TRUE, the function sets the SE_DACL_PRESENT flag in the
   *                            SECURITY_DESCRIPTOR_CONTROL
   *                            structure and uses the values in the pDacl and bDaclDefaulted
   *                            parameters. If this parameter
   *                            is FALSE, the function clears the SE_DACL_PRESENT flag, and pDacl
   *                            and bDaclDefaulted are ignored.
   * @param pDacl               A pointer to an ACL structure that specifies the DACL for the
   *                            security descriptor. If this
   *                            parameter is NULL, a NULL DACL is assigned to the security
   *                            descriptor, which allows all access
   *                            to the object. The DACL is referenced by, not copied into, the
   *                            security descriptor.
   * @param bDaclDefaulted      A flag that indicates the source of the DACL. If this flag is
   *                            TRUE, the DACL has been retrieved
   *                            by some default mechanism. If FALSE, the DACL has been explicitly
   *                            specified by a user. The function
   *                            stores this value in the SE_DACL_DEFAULTED flag of the
   *                            SECURITY_DESCRIPTOR_CONTROL structure. If
   *                            this parameter is not specified, the SE_DACL_DEFAULTED flag is
   *                            cleared.
   * @return If the function succeeds, the return value is nonzero. If the
   * function fails, the return value is zero. For extended error
   * information, call GetLastError.
   ...
   *
   * @param pAcl          A pointer to an ACL structure to be initialized by this function.
   *                      Allocate memory for pAcl before calling this function.
   * @param nAclLength    The length, in bytes, of the buffer pointed to by the pAcl parameter.
   *                      This value
   *                      must be large enough to contain the ACL header and all of the access
   *                      control
   ...
   *                      see Remarks.
   * @param dwAclRevision The revision level of the ACL structure being created. This value can
   *                      be ACL_REVISION
   *                      or ACL_REVISION_DS. Use ACL_REVISION_DS if the access control list
   *                      (ACL) supports
   ...
   * information, call GetLastError.
   */
  boolean AddAce(ACL pAcl, int dwAceRevision, int dwStartingAceIndex, Pointer pAceList,
      int nAceListLength);
   ...
   * control list (ACL).
   *
   * @param pAcl          A pointer to an ACL. This function adds an access-allowed ACE to the
   *                      end of this ACL.
   *                      The ACE is in the form of an ACCESS_ALLOWED_ACE structure.
   * @param dwAceRevision Specifies the revision level of the ACL being modified. This value can
   *                      be ACL_REVISION or
   *                      ACL_REVISION_DS. Use ACL_REVISION_DS if the ACL contains
   *                      object-specific ACEs.
   * @param AccessMask    Specifies the mask of access rights to be granted to the specified SID.
   * @param pSid          A pointer to the SID representing a user, group, or logon account being
   *                      granted access.
   * @return If the function succeeds, the return value is nonzero. If the
   * function fails, the return value is zero. For extended error
   * information, call GetLastError.
   ...
   *                      ACL_REVISION_DS. Use ACL_REVISION_DS if the ACL contains
   *                      object-specific ACEs.
   * @param AceFlags      A set of bit flags that control ACE inheritance. The function sets
   *                      these flags in the AceFlags
   *                      member of the ACE_HEADER structure of the new ACE. This parameter can
   *                      be a combination
   *                      of the following values: CONTAINER_INHERIT_ACE, INHERIT_ONLY_ACE,
   *                      INHERITED_ACE,
   ...
   * information, call GetLastError.
   */
  boolean AddAccessAllowedAceEx(ACL pAcl, int dwAceRevision, int AceFlags, int AccessMask,
      PSID pSid);
   ...
   * @param dwAceIndex The index of the ACE to be retrieved. A value of zero corresponds to the
   *                   first ACE in
   *                   the ACL, a value of one to the second ACE, and so on.
   * @param pAce       A pointer to a pointer that the function sets to the address of the ACE.
   * @return If the function succeeds, the return value is nonzero. If the
   * function fails, the return value is zero. For extended error
   * information, call GetLastError.
   */
   ...
   *
   * @param lpszUsername  A pointer to a null-terminated string that specifies the name
   ...
   * @param lpszDomain    A pointer to a null-terminated string that specifies the name
   ...
   *                      database.
   * @param lpszPassword  A pointer to a null-terminated string that specifies the
   ...
   * @param DesiredAccess Specifies an access mask that specifies the requested types of
   ...
   * @param OpenAsSelf    Indicates whether the access check is to be made against the
   *                      security context of the thread calling the OpenThreadToken
   *                      function or against the security context of the process for
   *                      the calling thread.
   * @param TokenHandle   Pointer to a variable that receives the handle to the newly
   *                      opened access token.
   * @return If the function succeeds, the return value is nonzero. If the
   ...
   * process.
   *
   * @param ProcessHandle Handle to the process whose access token is opened. The
   ...
   * @param TokenHandle   Pointer to a handle that identifies the newly opened access
   ...
   *
   * @param ExistingTokenHandle  Handle to an access token opened with TOKEN_DUPLICATE access.
   * @param ImpersonationLevel   Specifies a SECURITY_IMPERSONATION_LEVEL enumerated type that
   *                             supplies the impersonation level of the new token.
   * @param DuplicateTokenHandle Pointer to a variable that receives a handle to the duplicate
   ...
   * impersonation token.
   *
   * @param hExistingToken     A handle to an access token opened with TOKEN_DUPLICATE
   *                           access.
   * @param dwDesiredAccess    Specifies the requested access rights for the new token.
   * @param lpTokenAttributes  A pointer to a SECURITY_ATTRIBUTES structure that specifies a
   *                           security descriptor for the new token and determines whether
   *                           child processes can inherit the token.
   * @param ImpersonationLevel Specifies a value from the SECURITY_IMPERSONATION_LEVEL
   ...
   *                               access.
   * @param tokenInformationClass  Specifies a value from the TOKEN_INFORMATION_CLASS enumerated
   *                               type to identify the type of information the function
   *                               retrieves.
   * @param tokenInformation       Pointer to a buffer the function fills with the requested
   ...
   *                               parameter.
   * @param tokenInformationLength Specifies the size, in bytes, of the buffer pointed to by the
   *                               TokenInformation parameter. If TokenInformation is NULL, this
   *                               parameter must be zero.
   * @param returnLength           Pointer to a variable that receives the number of bytes needed
   ...
   * @param lpSubKey   Pointer to a null-terminated string containing the name of the
   *                   subkey to open.
   * @param ulOptions  Reserved; must be zero.
   * @param samDesired Access mask that specifies the desired access rights to the
   *                   key. The function fails if the security descriptor of the key
   *                   does not permit the requested access for the calling process.
   * @param phkResult  Pointer to a variable that receives a handle to the opened
   ...
   *
   * @param lpFile     The name of the hive file.
   *                   If the file does not exist, an empty hive file is created
   *                   with the specified name.
   * @param phkResult  Pointer to the handle for the root key of the loaded hive.
   * @param samDesired Access mask that specifies the desired access rights to the
   * @param dwOptions  If this parameter is REG_PROCESS_APPKEY,
   ...
   * computer.
   *
   * @param lpMachineName The name of the remote computer. The string has
   ...
   * of the error.
   * @see
   * <a href="https://msdn.microsoft.com/en-us/library/windows/desktop/ms724840.aspx">RegConnectRegistry function (Windows)</a>
   * @see
   * <a href="https://msdn.microsoft.com/en-us/library/windows/desktop/ms724836.aspx">Predefined Keys (Windows)</a>
   */
   ...
   * @param lpValueName Pointer to a null-terminated string containing the name of the
   ...
   * @param lpType      Pointer to a variable that receives a code indicating the type
   ...
   *
   * @param hKey Handle to the open key to be closed. The handle must have been
   ...
   * @param lpValueName Pointer to a null-terminated string that names the value to
   ...
   * @param hKey        Handle to an open key. The key must have been opened with the
   *                    KEY_SET_VALUE access right.
   * @param lpValueName Pointer to a string containing the name of the value to set.
   ...
   * @param lpData      Pointer to a buffer containing the data to be stored with the
   *                    specified value name.
   * @param cbData      Size of the information pointed to by the lpData parameter, in
   ...
   * @param dwIndex           Index of the subkey to retrieve. This parameter should be zero
   ...
   *                          the buffer.
   * @param lpcName           Pointer to a variable that specifies the size of the buffer
   ...
   * @param lpClass           Pointer to a buffer that receives the null-terminated class
   *                          string of the enumerated subkey. This parameter can be NULL.
   * @param lpcClass          Pointer to a variable that specifies the size of the buffer
   ...
   *                          lpClass is NULL.
   * @param lpftLastWriteTime Pointer to a variable that receives the time at which the
   ...
   * @param hKey           Handle to an open key. The key must have been opened with the
   *                       KEY_QUERY_VALUE access right.
   * @param dwIndex        Index of the value to be retrieved. This parameter should be
   ...
   * @param lpValueName    Pointer to a buffer that receives the name of the value,
   *                       including the terminating null character.
   * @param lpcchValueName Pointer to a variable that specifies the size of the buffer
   ...
   * @param lpClass                A pointer to a buffer that receives the null-terminated class
   *                               string of the key. This parameter can be ignored. This
   *                               parameter can be NULL.
   * @param lpcClass               A pointer to a variable that specifies the size of the buffer
   *                               pointed to by the lpClass parameter, in characters.
   * @param lpReserved             Reserved; must be NULL.
   * @param lpcSubKeys             A pointer to a variable that receives the number of subkeys
   *                               that are contained by the specified key. This parameter can be
   *                               NULL.
   * @param lpcMaxSubKeyLen        A pointer to a variable that receives the size of the key's
   *                               subkey with the longest name, in characters, not including the
   *                               terminating null character. This parameter can be NULL.
   * @param lpcMaxClassLen         A pointer to a variable that receives the size of the longest
   ...
   *                               parameter can be NULL.
   * @param lpcValues              A pointer to a variable that receives the number of values
   *                               that are associated with the key. This parameter can be NULL.
   * @param lpcMaxValueNameLen     A pointer to a variable that receives the size of the key's
   *                               longest value name, in characters. The size does not include
   *                               the terminating null character. This parameter can be NULL.
   * @param lpcMaxValueLen         A pointer to a variable that receives the size of the longest
   *                               data component among the key's values, in bytes. This
   *                               parameter can be NULL.
   * @param lpcbSecurityDescriptor A pointer to a variable that receives the size of the key's
   *                               security descriptor, in bytes. This parameter can be NULL.
   * @param lpftLastWriteTime      A pointer to a FILETIME structure that receives the last write
   ...
   *
   * @param hkey     [in] A handle to an open registry key. The key must have been
   ...
   * @param lpSubKey [in, optional] The name of the registry key. This key must be
   ...
   *                       .com/en-us/windows/desktop/SysInfo/predefined-keys">predefined
   *                       keys</a>. The key must have been opened with the
   *                       {@code KEY_NOTIFY} access right. For more information,
   *                       see <a href=
   *                       "https://docs.microsoft
   *                       .com/en-us/windows/desktop/SysInfo/registry-key-security-and-access
   *                       -rights">Registry
   ...
   *
   * @param lpUNCServerName The Universal Naming Convention (UNC) name of the remote
   *                        server on which this operation is to be performed. If this
   *                        parameter is NULL, the local computer is used.
   * @param lpSourceName    The name of the event source whose handle is to be retrieved.
   ...
   *
   * @param hEventLog A handle to the event log. The RegisterEventSource function
   *                  returns this handle.
   * @return If the function succeeds, the return value is nonzero. If the
   * function fails, the return value is zero. To get extended error
   * information, call GetLastError.
   ...
   *                        parameter is NULL, the local computer is used.
   * @param lpSourceName    The name of the log. If you specify a custom log and it cannot
   ...
   * Closes the specified event log.
   *
   * @param hEventLog A handle to the event log to be closed. The OpenEventLog or
   ...
   *                        OpenBackupEventLog function returns this handle.
   * @param NumberOfRecords A pointer to a variable that receives the number of records in
   ...
   * @param wType       The type of event to be logged.
   * @param wCategory   The event category. This is source-specific information; the
   *                    category can have any value.
   * @param dwEventID   The event identifier. The event identifier specifies the entry
   *                    in the message file associated with the event source.
   * @param lpUserSid   A pointer to the current user's security identifier. This
   *                    parameter can be NULL if the security identifier is not
   *                    required.
   * @param wNumStrings The number of insert strings in the array pointed to by the
   *                    lpStrings parameter. A value of zero indicates that no strings
   *                    are present.
   * @param dwDataSize  The number of bytes of event-specific raw (binary) data to
   *                    write to the log. If this parameter is zero, no event-specific
   *                    data is present.
   * @param lpStrings   A pointer to a buffer containing an array of null-terminated
   ...
   * @param lpRawData   A pointer to the buffer containing the binary data. This
   ...
   *
   * @param hEventLog        A handle to the event log to be cleared. The OpenEventLog
   *                         function returns this handle.
   * @param lpBackupFileName The absolute or relative path of the backup file. If this file
   ...
   * clear the event log.
   *
   * @param hEventLog        A handle to the open event log. The OpenEventLog function
   *                         returns this handle.
   ...
   *                                 function returns this handle.
   * @param dwReadFlags              Use the following flag values to indicate how to read the log
   *                                 file.
   * @param dwRecordOffset           The record number of the log-entry at which the read operation
   *                                 should start. This parameter is ignored unless dwReadFlags
   *                                 includes the EVENTLOG_SEEK_READ flag.
   * @param lpBuffer                 An application-allocated buffer that will receive one or more
   ...
   * @param nNumberOfBytesToRead     The size of the lpBuffer buffer, in bytes. This function will
   *                                 read as many log entries as will fit in the buffer; the
   *                                 function will not return partial entries.
   * @param pnBytesRead              A pointer to a variable that receives the number of bytes read
   *                                 by the function.
   * @param pnMinNumberOfBytesNeeded A pointer to a variable that receives the required size of the
   ...
   *
   * @param hEventLog    Handle to the open event log. This handle is returned by the
   *                     OpenEventLog or OpenBackupEventLog function.
   * @param OldestRecord Pointer to a variable that receives the absolute record number
   *                     of the oldest record in the specified event log.
   * @return If the function succeeds, the return value is nonzero. If the
   * function fails, the return value is zero. To get extended error
   * information, call GetLastError.
   ...
   *
   * @param hService    A handle to the service. This handle is returned by the
   ...
   * @param dwInfoLevel The configuration information to be changed.
   * @param lpInfo      A pointer to the new value to be set for the configuration
   ...
   *
   * @param hService       A handle to the service. This handle is returned by the OpenService or
   *                       CreateService function and must have the SERVICE_QUERY_CONFIG access
   *                       right. For
   *                       more information, see Service Security and Access Rights.
   * @param dwInfoLevel    The configuration information to be queried.
   * @param lpBuffer       A pointer to the buffer that receives the service configuration
   *                       information. The
   *                       format of this data depends on the value of the dwInfoLevel parameter.
   *                       The maximum size of this array is 8K bytes. To determine the required
   *                       size,
   *                       specify NULL for this parameter and 0 for the cbBufSize parameter. The
   *                       function
   *                       fails and GetLastError returns ERROR_INSUFFICIENT_BUFFER. The
   *                       pcbBytesNeeded
   *                       parameter receives the needed size.
   * @param cbBufSize      The size of the structure pointed to by the lpBuffer parameter, in bytes.
   * @param pcbBytesNeeded A pointer to a variable that receives the number of bytes required to
   *                       store the
   *                       configuration information, if the function fails with
   *                       ERROR_INSUFFICIENT_BUFFER.
   * @return If the function succeeds, the return value is nonzero.
   ...
   *                       href="http://msdn.microsoft.com/en-us/library/ms685981.aspx"
   *                       >Service Security and Access Rights</a>.
   * @param InfoLevel      The service attributes to be returned (a value from
   ...
   * @param lpBuffer       (optional) A pointer to the buffer that receives the status
   ...
   * @param cbBufSize      The size of the buffer pointed to by the lpBuffer parameter,
   *                       in bytes.
   * @param pcbBytesNeeded A pointer to a variable that receives the number of bytes
   ...
   *                        href="http://msdn.microsoft.com/en-us/library/ms685981.aspx"
   *                        >Service Security and Access Rights</a>.
   * @param lpServiceStatus A pointer to a SERVICE_STATUS structure that receives the status
   *                        information.
   * @return If the function succeeds, the return value is true. If the
   * function fails, the return value is false. To get extended error
   * information, call GetLastError.
   ...
   * @param dwControl       This parameter can be one of the following control codes
   ...
   * @param lpServiceStatus A pointer to a SERVICE_STATUS structure that receives the
   ...
   *                            href="http://msdn.microsoft.com/en-us/library/ms685981.aspx">
   *                            Service Security and Access Rights</a>.
   * @param dwNumServiceArgs    The number of strings in the lpServiceArgVectors array. If
   *                            lpServiceArgVectors is NULL, this parameter can be zero.
   * @param lpServiceArgVectors The null-terminated strings to be passed to the ServiceMain
   ...
   *
   * @param hSCObject A handle to the service control manager object or the service
   ...
   *
   * @param hSCManager      A handle to the service control manager database. The
   *                        OpenSCManager(String, String, int) function returns this
   *                        handle.
   * @param lpServiceName   The name of the service to be opened. This is the name
   ...
   * @param dwDesiredAccess The access to the service. For a list of access rights, see <a
   *                        href="http://msdn.microsoft.com/en-us/library/ms685981.aspx">
   ...
   *
   * @param lpMachineName   The name of the target computer. If the pointer is NULL or
   ...
   *                        default.
   * @param dwDesiredAccess The access to the service control manager. For a list of
   *                        access rights, see <a
   *                        href="http://msdn.microsoft.com/en-us/library/ms685981.aspx">
   *                        Service Security and Access Rights</a>. Before granting the
   ...
   *                           <p>
   *                           The order of the services in this array is the reverse of the
   *                           start order
   *                           of the services. In other words, the first service in the array is
   *                           the
   *                           one that would be started last, and the last service in the array
   *                           is the
   *                           one that would be started first.</p>
   *                           <p>
   *                           The maximum size of this array is 64,000 bytes. To determine the
   *                           required
   ...
   * invalid.</td></tr>
   * <tr><td>{@link WinError#ERROR_MORE_DATA}</td><td>The buffer pointed to by lpServices is not
   * large enough. The function sets the variable pointed to by lpServicesReturned to the actual
   * number of service entries stored into the buffer. The function sets the variable pointed to
   * by pcbBytesNeeded to the number of bytes required to store all of the service entries
   * .</td></tr>
   * </table>
   */
   ...
   *                           <tr><td>{@link WinNT#SERVICE_FILE_SYSTEM_DRIVER}</td><td>File
   *                           system driver services.</td></tr>
   ...
   *                           pcbBytesNeeded parameter will receive the required size.</p>
   *
   *                           <p><b>Windows Server 2003 and Windows XP:</b> The maximum size of
   *                           this array is 64K
   *                           bytes. This limit was increased as of Windows Server 2003 with SP1
   *                           and
   ...
   * <tr><td>{@link WinError#ERROR_INVALID_PARAMETER}</td><td>A parameter that was specified is
   * invalid.</td></tr>
   * <tr><td>{@link WinError#ERROR_INVALID_LEVEL}</td><td>The InfoLevel parameter contains an
   * unsupported value.</td></tr>
   * <tr><td>{@link WinError#ERROR_SHUTDOWN_IN_PROGRESS}</td><td>The system is shutting down;
   * this function cannot be called.</td></tr>
   * </table>
   */
   ...
   * @param lpProcessAttributes  A pointer to a SECURITY_ATTRIBUTES structure that specifies a
   ...
   *                             process.
   * @param lpThreadAttributes   A pointer to a SECURITY_ATTRIBUTES structure that specifies a
   ...
   *
   * @param TokenHandle          A handle to the access token that contains the privileges to
   *                             be modified.
   * @param DisableAllPrivileges Specifies whether the function disables all of the token's
   *                             privileges.
   * @param NewState             A pointer to a TOKEN_PRIVILEGES structure that specifies an
   *                             array of privileges and their attributes.
   * @param BufferLength         Specifies the size, in bytes, of the buffer pointed to by the
   ...
   * @param ReturnLength         A pointer to a variable that receives the required size, in
   ...
   *
   * @param lpSystemName A pointer to a null-terminated string that specifies the name
   ...
   * @param lpLuid       A pointer to the LUID by which the privilege is known on the
   *                     target system.
   * @param lpName       A pointer to a buffer that receives a null-terminated string
   *                     that represents the privilege name. For example, this string
   *                     could be "SeSecurityPrivilege".
   * @param cchName      A pointer to a variable that specifies the size, in a TCHAR
   ...
   *                     null string is specified, the function attempts to find the
   *                     privilege name on the local system.
   * @param lpName       A pointer to a null-terminated string that specifies the name
   ...
   *
   * @param lpFileName           A pointer to a null-terminated string that specifies the file
   *                             or directory for which security information is retrieved.
   * @param RequestedInformation A SECURITY_INFORMATION value that identifies the security
   *                             information being requested. See WinNT *_SECURITY_INFORMATION
   * @param pointer              A pointer to a buffer that receives a copy of the security
   ...
   *                             format.
   * @param nLength              Specifies the size, in bytes, of the buffer pointed to by the
   *                             pSecurityDescriptor parameter.
   * @param lpnLengthNeeded      A pointer to the variable that receives the number of bytes
   ...
   *
   * @param lpFileName          A pointer to a null-terminated string that specifies the file or
   *                            directory for which security is set.
   *                            Note that security applied to a directory is not inherited by its
   *                            children.
   * @param SecurityInformation Specifies a SECURITY_INFORMATION structure that identifies the
   *                            contents of the security descriptor
   *                            pointed to by the pSecurityDescriptor parameter.
   * @param pSecurityDescriptor A pointer to a SECURITY_DESCRIPTOR structure.
   * @return If the function succeeds, the function returns nonzero. If the function fails, it
   * returns zero. To get
  ...
  /**
   * The GetSecurityInfo function retrieves a copy of the security descriptor for an object
   ...
   *                             in the security descriptor returned
   *                             in ppSecurityDescriptor. The returned pointer is valid only if
   *                             you set the OWNER_SECURITY_INFORMATION flag.
   ...
   *                             group SID in the returned security descriptor.
   *                             The returned pointer is valid only if you set the
   *                             GROUP_SECURITY_INFORMATION flag. This parameter can be NULL
   *                             if you do not need the group SID.
   * @param ppDacl               [in, optional]
   *                             A pointer to a variable that receives a pointer to the DACL in
   *                             the returned security descriptor. The returned
   *                             pointer is valid only if you set the DACL_SECURITY_INFORMATION
   *                             flag. This parameter can be NULL if you do not
   *                             need the DACL.
   * @param ppSacl               [in, optional]
   *                             A pointer to a variable that receives a pointer to the SACL in
   *                             the returned security descriptor. The returned
   *                             pointer is valid only if you set the SACL_SECURITY_INFORMATION
   *                             flag. This parameter can be NULL if you do not
   *                             need the SACL.
   * @param ppSecurityDescriptor A pointer to a variable that receives a pointer to the security
   *                             descriptor of the object. When you have finished
   *                             using the pointer, free the returned buffer by calling the
   *                             LocalFree function.
   ...
   *
   * <p><b>NOTES:</b></p>
   * <p>1. If the ppsidOwner, ppsidGroup, ppDacl, and ppSacl parameters are non-NULL, and the
   * SecurityInfo parameter specifies
   * that they be retrieved from the object, those parameters will point to the corresponding
   * parameters in the security descriptor
   * returned in ppSecurityDescriptor.</p>
   * <p>2. To read the owner, group, or DACL from the object's security descriptor, the calling
   * process must have been granted
   * READ_CONTROL access when the handle was opened. To get READ_CONTROL access, the caller must
   * be the owner of the object or
   * the object's DACL must grant the access.</p>
   * <p>3. To read the SACL from the security descriptor, the calling process must have been
   * granted ACCESS_SYSTEM_SECURITY access
   * when the handle was opened. The proper way to get this access is to enable the
   * SE_SECURITY_NAME privilege in the caller's
   * current token, open the handle for ACCESS_SYSTEM_SECURITY access, and then disable the
   * privilege.</p>
   ...
   *
   * @param pObjectName          A pointer to a that specifies the name of the object from
   ...
   * @param ObjectType           Specifies a value from the SE_OBJECT_TYPE enumeration that
   *                             indicates the type of object named by the pObjectName parameter.
   * @param SecurityInfo         A set of bit flags that indicate the type of security
   *                             information to retrieve. See WinNT *_SECURITY_INFORMATION
   * @param ppsidOwner           [out, optional]
   *                             A pointer to a variable that receives a pointer to the owner SID
   ...
   *
   * @param ppSecurityDescriptor A pointer to the SECURITY_DESCRIPTOR structure whose length the
   *                             function returns.
   *                             The pointer is assumed to be valid.
   * @return If the function succeeds, the function returns the length, in bytes, of the
  ...
  /**
   * The IsValidSecurityDescriptor function determines whether the components of a security
   * descriptor are valid.
   *
   * @param ppSecurityDescriptor [in]
   *                             A pointer to a SECURITY_DESCRIPTOR structure that the function
   *                             validates.
  ...
  /**
   * A pointer to a SECURITY_DESCRIPTOR structure in absolute format. The function creates a
   * version of this security
   * descriptor in self-relative format without modifying the original.
   *
   * @param pAbsoluteSD      A pointer to a SECURITY_DESCRIPTOR structure in absolute format. The
   *                         function creates a version of this
   *                         security descriptor in self-relative format without modifying the
   *                         original.
   * @param pSelfRelativeSD  A pointer to a buffer the function fills with a security descriptor
   *                         in self-relative format.
   * @param lpdwBufferLength A pointer to a variable specifying the size of the buffer pointed to
   *                         by the pSelfRelativeSD parameter.
   *                         If the buffer is not large enough for the security descriptor, the
   *                         function fails and sets this variable
   *                         to the minimum required size.
   * @return If the function succeeds, the function returns nonzero. If the function fails, it
   * returns zero. To get
   * extended error information, call GetLastError. Possible return codes include, but are not
   ...
   *
   * @param pSelfRelativeSD      A pointer to a SECURITY_DESCRIPTOR structure in self-relative
   *                             format. The function creates an
   *                             absolute-format version of this security descriptor without
   *                             modifying the original security descriptor.
   * @param pAbsoluteSD          A pointer to a buffer that the function fills with the main body
   *                             of an absolute-format security
   *                             descriptor. This information is formatted as a
   *                             SECURITY_DESCRIPTOR structure.
   * @param lpdwAbsoluteSDSize   A pointer to a variable that specifies the size of the buffer
   *                             pointed to by the pAbsoluteSD parameter.
   *                             If the buffer is not large enough for the security descriptor,
   *                             the function fails and sets this variable
   *                             to the minimum required size.
   * @param pDacl                A pointer to a buffer the function fills with the discretionary
   *                             access control list (DACL) of the
   *                             absolute-format security descriptor. The main body of the
   *                             absolute-format security descriptor references
   *                             this pointer.
   * @param lpdwDaclSize         A pointer to a variable that specifies the size of the buffer
   *                             pointed to by the pDacl parameter. If
   *                             the buffer is not large enough for the access control list (ACL)
   *                             , the function fails and sets this
   *                             variable to the minimum required size.
   * @param pSacl                A pointer to a buffer the function fills with the system access
   *                             control list (SACL) of the absolute-format
   *                             security descriptor. The main body of the absolute-format
   *                             security descriptor references this pointer.
   * @param lpdwSaclSize         A pointer to a variable that specifies the size of the buffer
   *                             pointed to by the pSacl parameter. If the
   *                             buffer is not large enough for the ACL, the function fails and
   *                             sets this variable to the minimum required
   *                             size.
   * @param pOwner               A pointer to a buffer the function fills with the security
   *                             identifier (SID) of the owner of the
   *                             absolute-format security descriptor. The main body of the
   *                             absolute-format security descriptor references
   *                             this pointer.
   * @param lpdwOwnerSize        A pointer to a variable that specifies the size of the buffer
   *                             pointed to by the pOwner parameter.
   *                             If the buffer is not large enough for the SID, the function
   *                             fails and sets this variable to the minimum
   *                             required size.
   * @param pPrimaryGroup        A pointer to a buffer the function fills with the SID of the
   *                             absolute-format security descriptor's
   *                             primary group. The main body of the absolute-format security
   *                             descriptor references this pointer.
   * @param lpdwPrimaryGroupSize A pointer to a variable that specifies the size of the buffer
   *                             pointed to by the pPrimaryGroup parameter.
   *                             If the buffer is not large enough for the SID, the function
   *                             fails and sets this variable to the minimum
   *                             required size.
   * @return If the function succeeds, the function returns nonzero. If the function fails, it
   ...
   *
   * @param pAcl [in]
   *             A pointer to an ACL structure validated by this function. This value must not be
   *             NULL.
   * @return If the ACL is valid, the function returns nonzero. If the ACL is not valid, the
   * function returns zero.
   * There is no extended error information for this function; do not call GetLastError.
   * <p>
   * This function checks the revision level of the ACL and verifies that the number of access
   * control entries
   * (ACEs) specified in the AceCount member of the ACL structure fits the space specified by the
   * AclSize member
   ...
   * @param GenericMapping [in] A pointer to a GENERIC_MAPPING structure specifying a mapping of
   ...
   *
   * @param pSecurityDescriptor [in] A pointer to a SECURITY_DESCRIPTOR structure against which
   *                            access is checked.
   * @param ClientToken         [in] A handle to an impersonation token that represents the
   *                            client that is attempting to gain access. The handle must have
   *                            TOKEN_QUERY access to the token; otherwise, the function fails
   *                            with ERROR_ACCESS_DENIED.
   * @param DesiredAccess       [in] Access mask that specifies the access rights to check. This
   *                            mask must have been mapped by the MapGenericMask function to
   *                            contain no generic access rights.<br>
   *                            If this parameter is MAXIMUM_ALLOWED, the function sets the
   *                            GrantedAccess access mask to indicate the maximum access rights
   *                            the security descriptor allows the client.
   * @param GenericMapping      [in] A pointer to the GENERIC_MAPPING structure associated with
   *                            the object for which access is being checked.
   * @param PrivilegeSet        [out, optional] A pointer to a PRIVILEGE_SET structure that
   *                            receives the privileges used to perform the access validation. If
   *                            no privileges were used, the function sets the PrivilegeCount
   *                            member to zero.
   * @param PrivilegeSetLength  [in, out] Specifies the size, in bytes, of the buffer pointed to
   *                            by the PrivilegeSet parameter.
   * @param GrantedAccess       [out] A pointer to an access mask that receives the granted
   *                            access rights. If AccessStatus is set to FALSE, the function sets
   *                            the access mask to zero. If the function fails, it does not set
   *                            the access mask.
   * @param AccessStatus        [out] A pointer to a variable that receives the results of the
   *                            access check. If the security descriptor allows the requested
   *                            access rights to the client identified by the access token,
   *                            AccessStatus is set to TRUE. Otherwise, AccessStatus is set to
   *                            FALSE, and you can call GetLastError to get extended error
   *                            information.
   ...
   *
   * @param lpFileName The name of the file or directory to be encrypted.
   * @return If the function succeeds, the return value is nonzero. If the
   * function fails, the return value is zero. To get extended error
   * information, call GetLastError.
   */
   ...
   *
   * @param lpFileName The name of the file or directory to be decrypted.
   ...
   * @param lpStatus   A pointer to a variable that receives the encryption status of the
   *                   file.
   * @return If the function succeeds, the return value is nonzero. If the
   * function fails, the return value is zero. To get extended error
   * information, call GetLastError.
   ...
   * directory.
   *
   * @param DirPath The name of the directory for which to enable or disable
   *                encryption.
   * @param Disable Indicates whether to disable encryption (TRUE) or enable it
   *                (FALSE).
   * @return If the function succeeds, the return value is nonzero. If the
   * function fails, the return value is zero. To get extended error
   * information, call GetLastError.
   ...
   *
   * @param lpFileName The name of the file to be opened. The string must consist of
   *                   characters from the Windows character set.
   * @param ulFlags    The operation to be performed.
   * @param pvContext  The address of a context block that must be presented in subsequent
   ...
   *
   * @param pfExportCallback  A pointer to the export callback function. The system calls the
   ...
   * @param pvCallbackContext A pointer to an application-defined and allocated context block.
   ...
   * @param pvContext         A pointer to a system-defined context block. The context block is
   ...
   *
   * @param pfImportCallback  A pointer to the import callback function. The system calls the
   ...
   * functionality, while maintaining files in their encrypted state.
   *
   * @param pvContext A pointer to a system-defined context block. The
   ...
   * @param lpApplicationName  [in, optional]<br>
   ...
   *                           arguments.
   * @param lpCommandLine      [in, out, optional]<br>
   ...
   * @param dwCreationFlags    The flags that control how the process is created. <br>
   ...
   *                           supported.<br>
   * @param lpEnvironment      [in, optional]<br>
   ...
   * @param lpCurrentDirectory [in, optional]<br>
   ...
   * <table>
   * <tr><th>Return code</th><th>Description</th></tr>
   * <tr><td>ERROR_FAILED_SERVICE_CONTROLLER_CONNECT</td><td>This error is returned if the
   * program is being run as a console application rather than as a service. If the program will
   * be run as a console application for debugging purposes, structure it such that
   * service-specific code is not called when this error is returned.</td></tr>
   * <tr><td>ERROR_INVALID_DATA</td><td>The specified dispatch table contains entries that are
   * not in the proper format.</td></tr>
   * <tr><td>ERROR_SERVICE_ALREADY_RUNNING</td><td>The process has already called
   * StartServiceCtrlDispatcher. Each process can call StartServiceCtrlDispatcher only one time
   * .</td></tr>
   * </table>
   */
   ...
   * <table>
   * <tr><th>Return code</th><th>Description</th></tr>
   * <tr><td>ERROR_NOT_ENOUGH_MEMORY</td><td>Not enough memory is available to convert an ANSI
   * string parameter to Unicode. This error does not occur for Unicode string parameters.</td></tr>
   * <tr><td>ERROR_SERVICE_NOT_IN_EXE</td><td>The service entry was specified incorrectly when
   * the process called the {@link #StartServiceCtrlDispatcher} function.</td></tr>
   * </table>
   */
   ...
   *                           <tr><td>SERVICE_FILE_SYSTEM_DRIVER<br>0x00000002</td><td>File
   *                           system driver service.</td></tr>
   *                           <tr><td>SERVICE_KERNEL_DRIVER<br>0x00000001</td><td>Driver service
   *                           .</td></tr>
   *                           <tr><td>SERVICE_RECOGNIZER_DRIVER<br>0x00000008</td><td>Reserved
   *                           .</td></tr>
   *                           <tr><td>SERVICE_WIN32_OWN_PROCESS<br>0x00000010</td><td>Service
   *                           that runs in its own process.</td></tr>
   *                           <tr><td>SERVICE_WIN32_SHARE_PROCESS<br>0x00000020</td><td>Service
   *                           that shares a process with one or more other services. For more
   *                           information, see Service Programs.</td></tr>
   *                           </table>
   *
   *                           <p>If you specify either SERVICE_WIN32_OWN_PROCESS or
   *                           SERVICE_WIN32_SHARE_PROCESS, and the service is running in the
   *                           context of the LocalSystem account, you can also specify the
   *                           following value.</p>
   *
   *                           <table>
   *                           <tr><th>Value</th><th>Meaning</th></tr>
   *                           <tr><td>SERVICE_INTERACTIVE_PROCESS<br>0x00000100</td><td>The
   *                           service can interact with the desktop.</td></tr>
   *                           </table>
   ...
   *                           <table>
   *                           <tr><th>Value</th><th>Meaning</th></tr>
   *                           <tr><td>SERVICE_AUTO_START<br>0x00000002</td><td>A service started
   *                           automatically by the service control manager during system
   *                           startup.</td></tr>
   *                           <tr><td>SERVICE_BOOT_START<br>0x00000000</td><td>A device driver
   *                           started by the system loader. This value is valid only for driver
   *                           services.</td></tr>
   *                           <tr><td>SERVICE_DEMAND_START<br>0x00000003</td><td>A service
   *                           started by the service control manager when a process calls the
   *                           StartService function.</td></tr>
   *                           <tr><td>SERVICE_DISABLED<br>0x00000004</td><td>A service that
   *                           cannot be started. Attempts to start the service result in the
   *                           error code ERROR_SERVICE_DISABLED.</td></tr>
   *                           <tr><td>SERVICE_SYSTEM_START<br>0x00000001</td><td>A device driver
   *                           started by the IoInitSystem function. This value is valid only
   *                           for driver services.</td></tr>
   *                           </table>
   ...
   *                           <table>
   *                           <tr><th>Value</th><th>Meaning</th></tr>
   *                           <tr><td>SERVICE_ERROR_CRITICAL<br>0x00000003</td><td>The startup
   *                           program logs the error in the event log, if possible. If the
   *                           last-known-good configuration is being started, the startup
   *                           operation fails. Otherwise, the system is restarted with the
   *                           last-known good configuration.</td></tr>
   *                           <tr><td>SERVICE_ERROR_IGNORE<br>0x00000000</td><td>The startup
   *                           program ignores the error and continues the startup operation
   *                           .</td></tr>
   *                           <tr><td>SERVICE_ERROR_NORMAL<br>0x00000001</td><td>The startup
   *                           program logs the error in the event log but continues the startup
   *                           operation.</td></tr>
   *                           <tr><td>SERVICE_ERROR_SEVERE<br>0x00000002</td><td>The startup
   *                           program logs the error in the event log. If the last-known-good
   ...
   *
   *                           <p>HKEY_LOCAL_MACHINE\System\CurrentControlSet\Control
   *                           \ServiceGroupOrder</p>
   ...
   *
   *                           <p><strong>Windows Server 2008, Windows Vista, Windows Server 2003
   *                           and Windows XP:</strong>
   ...
   * <table>
   * <tr><th>Return code</th><th>Description</th></tr>
   * <tr><td>ERROR_ACCESS_DENIED</td><td>The handle to the SCM database does not have the
   * SC_MANAGER_CREATE_SERVICE access right.</td></tr>
   * <tr><td>ERROR_CIRCULAR_DEPENDENCY</td><td>A circular service dependency was specified
   * .</td></tr>
   ...
   * <tr><td>ERROR_INVALID_HANDLE</td><td>The specified handle is invalid.</td></tr>
   * <tr><td>ERROR_SERVICE_MARKED_FOR_DELETE</td><td>The specified service has already been
   * marked for deletion.</td></tr>
   * </table>
   */
