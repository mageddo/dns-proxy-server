...
public class ContainerHostnameMatcher {
  static Predicate<InspectContainerResponse> buildPredicate(Hostname host) {
    return buildPredicate(host, Configs.getInstance());
  }

  static Predicate<InspectContainerResponse> buildPredicate(Hostname host, final Config config) {
    return container -> {
